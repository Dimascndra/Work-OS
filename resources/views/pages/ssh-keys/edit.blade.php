<x-metrolar-layout title="Edit SSH Key">
    <x-card title="Edit SSH Key">
        <x-slot:toolbar>
        ...
        <form action="{{ route('ssh-keys.update', $sshKey) }}" method="POST">
            @csrf
            @method('PUT')
            ...
            <div class="form-group row">
                <div class="col-lg-6">
                    ...
                    <input type="text" name="ip_server" class="form-control" placeholder="Enter IP Address"
                        value="{{ old('ip_server', $sshKey->ip_server) }}" required />
                    @error('ip_server')
                        <span class="text-danger">{{ $message }}</span>
                    @enderror
                </div>
                <div class="col-lg-6">
                    ...
                    <input type="number" name="port" class="form-control" placeholder="Enter Port"
                        value="{{ old('port', $sshKey->port) }}" required />
                    @error('port')
                        <span class="text-danger">{{ $message }}</span>
                    @enderror
                </div>
            ...
            <div class="form-group row">
                <div class="col-lg-6">
                    ...
                    <input type="text" name="username" class="form-control" placeholder="Enter Username"
                        value="{{ old('username', $sshKey->username) }}" required />
                    @error('username')
                        <span class="text-danger">{{ $message }}</span>
                    @enderror
                </div>
                <div class="col-lg-6">
                    ...
                    <input type="password" name="password" class="form-control" placeholder="Enter Password (Optional)"
                        value="{{ old('password', $sshKey->password) }}" />
                    @error('password')
                        <span class="text-danger">{{ $message }}</span>
                    @enderror
                </div>
            ...
            <div class="form-group">
                ...
                <textarea name="public_key" class="form-control" rows="5" placeholder="Enter Public Key">{{ old('public_key', $sshKey->public_key) }}</textarea>
                @error('public_key')
                    <span class="text-danger">{{ $message }}</span>
                @enderror
            </div>
            ...
            <div class="card-footer text-right">
                ...
                <a href="{{ route('ssh-keys.index') }}" class="btn btn-secondary">Cancel</a>
            </div>
        </form>
    </x-card>
</x-metrolar-layout>
